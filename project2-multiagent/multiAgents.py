...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def min_value(state, depth, ghost_index, alpha, beta):
            ...
            for action in state.getLegalActions(ghost_index):
                if ghost_index == num_ghosts:
                    v = min(v, max_value(state.generateSuccessor(ghost_index, action), depth+1, alpha, beta)[0])
                else:
                    v = min(v, min_value(state.generateSuccessor(ghost_index, action), depth, ghost_index+1, alpha, beta))
                #prune 
                if v < alpha:
                    return v
                beta = min(beta, v)
